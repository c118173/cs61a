...
def pig_pass(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4b
    if -3<player_score-opponent_score<0:
        return True
    else:
        return False
    # END PROBLEM 4b
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    def procedure(player):
        if player==0:
            strategy,score,op_score=strategy0,score0,score1
        else:
            strategy,score,op_score=strategy1,score1,score0
        
        score += take_turn(strategy(score,op_score),op_score,dice)

        if not extra_turn(score,op_score):
            player=other(player)
        return score,player

    while score0<goal and score1<goal:
        if who:
            score1,who=procedure(who)
        else:
            score0,who=procedure(who)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def highestscore(score0,score1):
        if who:
            score=score1
        else:
            score=score0
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for number in range (1,11):
        num_score=make_averaged(roll_dice,trials_count)
        every_score=num_score(number,dice)
        if every_score > max_score:
            max_score=every_score
            max_rolls=number
    return max_rolls
    # END PROBLEM 9
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    trig_score = free_bacon(opponent_score)
    score += trig_score
    if extra_turn(score,opponent_score) or trig_score>=cutoff:
        return 0
    else:
        return num_rolls
