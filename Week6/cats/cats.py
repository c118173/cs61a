...
def about(topic):
    ...
    def select(para):
        ...
        para=split(para)
        if any(x in para for x in topic):
            return True
        else:
            return False
    return select
...
def accuracy(typed, reference):
    ...
    if not typed_words:
        return 0.0
    r=min(len(typed_words),len(reference_words))
    j,correct=0,0
    for i in range(r):
...
def pawssible_patches(start, goal, limit):
    ...
    if limit<0: # Fill in the condition
        ...
        if start[0]!=goal[0]:
            ...
            add_diff=remove_diff=substitute_diff=pawssible_patches(start[1:],goal[1:],limit)
        # BEGIN
...
def time_per_word(times_per_player, words):
    ...
    for i in range(len(times_per_player)):
        times_times += [[times_per_player[i][j]-times_per_player[i][j-1] for j in range(1,len(times_per_player[i]))]]
    return game(words,times_times)
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    words_words=[[] for _ in player_indices]
    for i in word_indices:
        fast_time,fast_player=99999,0
        for j in player_indices:
            if fast_time>time(game,j,i):
                fast_time=time(game,j,i)
                fast_player=j
        words_words[fast_player] += [word_at(game,i)]
    return words_words
    # END PROBLEM 10
