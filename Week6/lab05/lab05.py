...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    corrd_city=make_city('corrdinate',lat,lon)
    d1,d2=distance(city_a,corrd_city),distance(city_b,corrd_city)
    if d1<d2:
...
def add_trees(t1, t2):
    ...
    bran_t1,bran_t2=branches(t1),branches(t2)
    if l1<l2:
        bran_t1 += [None for _ in range(l1,l2)]
    elif l1>l2:
        bran_t2 += [None for _ in range(l2,l1)]
    return tree(label(t1)+label(t2),[add_trees(b1,b2) 
            for b1,b2 in zip(bran_t1,bran_t2)])
