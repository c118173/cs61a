...
def replace_leaf(t, find_value, replace_value):
    ...
    lst=[replace_leaf(b,find_value,replace_value) for b in branches(t)]
...
def preorder(t):
    ...
    if is_leaf(t):
        return [label(t)]
    lst=[preorder(b) for b in branches(t)]
    return [label(t)]+sum(lst,[])
...
def sub_interval(x, y):
    ...
    and any value in y."""
    my=interval(-upper_bound(y),-lower_bound(y))
    return add_interval(x,my)
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert upper_bound(y)*lower_bound(y)>0, "y can not span zaro"
...
def multiple_references_explanation():
    return """The multiple reference problem exists. Refer to the same interval multiple times may result
    in intervals that are larger then they should be."""
...
def quadratic(x, a, b, c):
    ...
    if lower_bound(x) < -b/(2*a) < upper_bound(x):
        return interval(min(point,f1,f2), max(point,f1,f2))
    else:
