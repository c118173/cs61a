...
def prune_min(t):
    ...
    """
    if t.branches:
        b1 = t.branches[0].label
        b2 = t.branches[1].label
        if b1 < b2:
...
def num_splits(s, d):
    ...
    def helper(s, s1, s2):
        if s:
            return helper(s[1:], s1+[s[0]], s2) + helper(s[1:], s1, s2+[s[0]])
        if abs(sum(s1)-sum(s2)) <= d:
            return 1
        else:
            return 0
    return helper(s, [], [])//2
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        possibilities.append(("+["+code_char+"]"+match, cost+1))
        # Delete
        match, cost = helper_align(skeleton_idx+1, code_idx)
        possibilities.append(("-["+skel_char+"]"+match, cost+1))
